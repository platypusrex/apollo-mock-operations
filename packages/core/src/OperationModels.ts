...
export class OperationModels<TModels extends Record<string, OperationModel<any>>> {
  ...
  private getInitialModelData = <K extends keyof OperationType<any, any>>(
    ...
    if (!isSSR()) {
      ...
      if (persistedModelState) {
        const parsedModelState = parseJSON<Record<K, NonEmptyArray<ResolverReturnType<OperationType<any, any>[K]>>>>(
          persistedModelState
        );
  ...
  createModel = <K extends keyof OperationType<any, any>>(
    ...
    this.models = {
      ...this.models,
      [name]: new OperationModel<OperationType<any, any>>(name, this.getInitialModelData(name, data)),
    };
    return this.models;
  };
